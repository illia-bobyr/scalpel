...
-- focused node.
--
-- That this function will move to the previous sibling node that is either an
-- open tag node or a text node.  In most cases, HTML documents will have text
-- nodes in-between element nodes, just due to formatting.  You may want to use
...
-- focused node.
--
-- That this function will move to the next sibling node that is either an open
